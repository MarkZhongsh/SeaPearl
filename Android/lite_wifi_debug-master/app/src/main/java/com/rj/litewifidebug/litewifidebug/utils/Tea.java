...
{
	// 封装后的XXTea加密算法接口
	public static byte[] DoTeaEncrypt(final String src, final String key){
		if( src == null )
			return null;
		...
		return TeaEncrypt(srcByte, srcByte.length, key.getBytes());
	}
	
	// XXTea算法的原始加密接口
	public static byte[] TeaEncrypt(final byte[] src, int srcLen, final byte[] key){
	...
	public static byte[] DoTeaDecrypt(final byte[] src, int srcLen, final byte[] key){
		return TeaDecrypt(src, srcLen, key);
	}
	
	public static byte[] TeaDecrypt(final byte[] src, int srcLen, final byte[] key){
		if(src == null || src.length <= 0 || srcLen <= 0)
			{
			return null;}
